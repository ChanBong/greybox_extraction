...
train_kinetics = datasets.Kinetics("../kinetics400_5per", frames_per_clip= 16, split='train', num_classes= '400', step_between_clips= 200000, transform = train_transform,  download= False, num_download_workers= 1, num_workers= 80)
...
def train_with_extraction(model, victim):
    ...
    for idx in range(10):
      ...
      for step,(video, label) in enumerate(train_dl):
          ...
          video = size_changer(video, 16, 256)
          pred = model(video)
